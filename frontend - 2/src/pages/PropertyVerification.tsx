...
import { OwnershipDocumentKey, Property } from '@/types';
import { ArrowLeft, AlertCircle, Check, FileText, Info, Upload, X } from 'lucide-react';
import { toast } from 'sonner';

type DocumentStatus = 'pending' | 'uploading' | 'uploaded' | 'invalid';
...
const requiredDocuments: { key: OwnershipDocumentKey; label: string }[] = [
...
export default function PropertyVerification() {
  ...
  const state = location.state as { propertyId?: string; propertyData?: Partial<Property> } | undefined;
  const propertyFromState = state?.propertyId;
  const propertyDataFromState = state?.propertyData;
  
  // Try to find property from mock data first, otherwise use data from state
  const property = useMemo(() => {
    const foundInMock = mockProperties.find((p) => p.id === id) ?? mockProperties.find((p) => p.id === propertyFromState);
    if (foundInMock) return foundInMock;
    
    // If not found in mock (new property), use data from state
    if (propertyDataFromState) {
      return propertyDataFromState as Property;
    }
    
    return undefined;
  }, [id, propertyFromState, propertyDataFromState]);
  const [documents, setDocuments] = useState<Record<OwnershipDocumentKey, DocumentState>>(() =>
    requiredDocuments.reduce((acc, doc) => {
      acc[doc.key] = { file: null, status: 'pending', progress: 0 };
      return acc;
    }, {} as Record<OwnershipDocumentKey, DocumentState>)
  );
  ...
  const handleDocumentSelect = (key: OwnershipDocumentKey, files: FileList | null) => {
    const file = files?.[0];
    if (!file) return;
  ...
  const handleRemove = (key: OwnershipDocumentKey) => {
    setDocuments((prev) => ({
      ...prev,
  ...
  const handleSubmit = async () => {
    ...
    toast.success('Ownership documents submitted. Verification is pending.');
    navigate('/properties');
  };
      ...
      <div className="space-y-6 animate-fade-in max-w-4xl mx-auto">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Upload all required documents to complete the verification process and verify you are the legal owner or authorized representative.
            </CardDescription>
          </CardHeader>
            ...
            {isRejected && (
              <div className="flex items-start gap-3 rounded-lg border border-destructive/30 bg-destructive/10 px-4 py-3 text-destructive">
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {rejectionReason || 'Please re-upload the documents for re-review.'}
                  </p>
                </div>
              </div>
            )}
                ...
                return (
                  <div key={doc.key} className="space-y-3">
                    ...
                    <div
                      ...
                      onClick={() => (document.getElementById(`${doc.key}-input`) as HTMLInputElement | null)?.click()}
                      onDragOver={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                      }}
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground">
                        Drag and drop PDF, JPG, or PNG files here.
                      </p>
                    </div>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground mt-1">
                              {Math.round(docState.file.size / 1024)} KB
                            </p>
                          </div>
                          <button
                            type="button"
