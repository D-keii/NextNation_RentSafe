...
export default function SquareListingCard({propertyId , imageUrl , monthlyRental , name , location , noOfBed , noOfToilet ,noOfSqft , isBookMarked}){
    return(
        <div className="bg-white border-1 w-fit rounded-lg overflow-hidden" >
            <div className="relative aspect-video max-w-[500px] overflow-hidden">
                <img src={imageUrl} onError={(e) => e.target.src = propertyImage} className="h-full w-full object-cover"/>
            ...
            <div className="p-3 flex flex-col space-y-2">
                ...
                <Link to={`/listing/${propertyId}`}className="flex flex-row border-1 w-full items-center justify-center p-2 rounded-md bg-muted cursor-pointer hover:bg-accent hover:text-white">View Details <ArrowRight className="ml-2"/></Link>
            </div>
        </div>
    )
}